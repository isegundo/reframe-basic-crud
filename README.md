...
The `debug?` variable in [`config.cljs`](src/cljs/reframe_basic_crud/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns reframe-basic-crud.example
  (:require [reframe-basic-crud.config :as config])
